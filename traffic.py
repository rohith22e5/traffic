...
def load_data(data_dir):
    ...
    folders=[int(content) for content in os.listdir(directory)]
    folders.sort()
    for folder in folders:
        for file in os.listdir(os.path.join(data_dir,str(folder))):    
            image=cv2.imread(os.path.join(data_dir,str(folder),file))         
            image= cv2.resize(image,(IMG_WIDTH,IMG_HEIGHT))     
            images.append(image)
            labels.append(folder)
            print(f"Image {file} done")  
        print(f"Folder {folder} done")  
    returned_values=(images,labels)
